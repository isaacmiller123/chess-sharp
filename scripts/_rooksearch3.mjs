...
const out=[]
const bk='h8'
for(const tF of 'abcdefgh') for(const tR of [8]){ const tg=files[files.indexOf(tF)]+tR
  if(dist(bk,tg)<4) continue
  for(const rF of 'abcdefgh') for(const rR of [1,2,3,4,5,6]){ const rk=files[files.indexOf(rF)]+rR
    ...
    let winMoves=[]; for(const [from,set] of p.allDests()) for(const to of set){ const e=evalMove(p,{from,to}); if(e.win>=3) winMoves.push({from,to,...e}) }
    if(winMoves.length!==1) continue
    const w=winMoves[0]; if(!w.gaveCheck||w.immCap>=3||p.board.get(w.from).role!=='rook') continue
    const san=makeSan(p.clone(),{from:w.from,to:w.to})
    out.push({fen,tg,rk,san,land:sq(w.to%8,Math.floor(w.to/8))}) }
}
console.log('UNIQUE rook forks BK h8:',out.length)
for(const r of out.slice(0,6)) console.log(`  ${r.san} land ${r.land}  N@${r.tg} R@${r.rk}  ${r.fen}`)
