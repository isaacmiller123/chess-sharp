...
const out=[]
for(const bk of ['g8','b8','h8','a8','b7','g7']){
...
const byBk={}; for(const r of out){ (byBk[r.bk]=byBk[r.bk]||[]).push(r) }
for(const bk of Object.keys(byBk)){ const r=byBk[bk][0]; console.log(`BK ${bk}: ${byBk[bk].length}; e.g. ${r.san} R@${r.rk} B@${r.b} land ${r.land} -> ${r.fen}`) }
