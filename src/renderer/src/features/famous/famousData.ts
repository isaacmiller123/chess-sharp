// Pure helpers for the Famous-games feature: PGN assembly for engine review,
// group ordering/labels, and result presentation. No React, no IPC.
import type { FamousGameDetail, FamousGameMeta, FamousGroup, FamousMove } from '@shared/types'
...
/**
 * Build a minimal but valid PGN from the detail's SAN moves so it can be fed to
 * window.api.review.run({ pgn }). Headers carry the players/event for context.
 */
...
/** FEN to show for a given cursor (0 = start position, i = after move i). */
export function fenAtCursor(moves: FamousMove[], cursor: number, initialFen: string): string {
  if (cursor <= 0) return initialFen
  const m = moves[Math.min(cursor, moves.length) - 1]
  return m ? m.fenAfter : initialFen
}
