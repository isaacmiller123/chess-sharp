...
/** Search depth for a sub-floor bot: 4 (Elo ~100) up to 8 (~1250+). */
function weakDepth(elo: number): number {
  return Math.round(lerpByElo(elo, 100, 4, 1250, 8))
}
...
/**
 * Softmax temperature in centipawns: ~650 at Elo 100 (near-uniform over the
 * candidates, real mistakes included) tapering to ~50 by 1250 (mostly top-2).
 */
function weakTemperature(elo: number): number {
  return lerpByElo(elo, 100, 650, 1250, 50)
}

/** Chance of a "human blunder": pick from the bottom half of the candidates. */
function weakBlunderChance(elo: number): number {
  return lerpByElo(elo, 100, 0.25, 1300, 0.03)
}
...
/** Softmax pick over candidates (sorted best-first) at the given temperature. */
function softmaxPick(cands: WeakCandidate[], temperature: number): string {
  const maxCp = cands[0].cp
  const weights = cands.map((c) => Math.exp((c.cp - maxCp) / temperature))
...
async function weakPlay(eng: UciEngine, fen: string, elo: number, panic = false): Promise<BestMove> {
  ...
  if (cands.length >= 2 && Math.random() < blunderChance) {
    ...
    return { bestmove: bottom[Math.floor(Math.random() * bottom.length)].uci }
  }
  return { bestmove: softmaxPick(cands, weakTemperature(elo) * (panic ? 1.7 : 1)) }
}
